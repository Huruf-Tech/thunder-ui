...
export const filesFromUrls = (urls?: string | string[]): FileMetadata[] => {
  if (!urls) return []
  ...
  return (urls instanceof Array ? urls : [urls]).map((rawUrl) => {
    ...
    const id = url.searchParams.get("fileId") ?? rawUrl
    const size = parseInt(url.searchParams.get("fileSize") ?? "0")
    const type = url.searchParams.get("fileType") ?? "*"

    url.searchParams.delete("fileId")
    url.searchParams.delete("fileSize")
...
export const urlsFromFiles = (files: FileWithPreview[]): string[] => {
  return files.map((file) => {
    ...
    url.searchParams.set("fileSize", file.file.size.toString())
    url.searchParams.set("fileType", file.file.type)
...
export function TableUpload({
  ...
  const defaultUploadFiles: FileUploadItem[] = (initialFiles ?? []).map(
    (file) => ({
      id: file.id,
      file: {
        name: file.name,
