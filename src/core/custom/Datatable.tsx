  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-1 flex-col rounded-xl border overflow-hidden">
      <TableVirtuoso
        ...
        components={{
          Scroller: (props) => {
            return (
              <div
                {...props}
                className="relative h-full min-h-0 w-full overflow-auto [&_tr:last-child]:border-b-0"
              />
            )
          },
          Table: ({ style, ...props }) => {
              ...
              >
                {row.getVisibleCells().map((cell) => {
                  ...
                  return (
                    <TableCell
                      key={cell.id}
                      className="relative truncate bg-background py-1 align-middle group-data-[state=selected]:border-background data-pinned:border-primary!"
                      data-last-col={
                        isLastLeftPinned
                          ? "left"
                          : isFirstRightPinned
        ...
        //     <TableRow
        //       key={footerGroup.id}
        //       className="sticky bottom-0 bg-muted/30 backdrop-blur-sm"
        //     >
